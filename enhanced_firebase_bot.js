...
async function syncUserToFirebase(member, message = null) {
  ...
  try {
    ...
    // Detect device/client info if message is provided
    let deviceInfo = "Unknown";
    if (message && message.client) {
      // Discord.js doesn't provide granular device info easily, 
      // but we can log the presence status if available
    ...
    const docRef = db.collection("discord_users").doc(member.user.id); // Use ID as primary key
...
/**
 * New function to store message history in Firebase
 */
...
/**
 * Refactored to use Genkit (Google AI) and store history in Firebase
 */
...
client.on(Events.MessageCreate, async message => {
  ...
  // Sync user data on every interaction to ensure it's up to date
  if (message.member) {
