// UserProfile.js — ESM, Firestore-based, safe for Railway
...
export async function getOrCreateAthenaUser(discordUser) {
  ...
  // --- 1️⃣ Fast path: existing Discord ID ---
  const existingDoc = await accountsRef.doc(discordUser.id).get();
  if (existingDoc.exists) {
    return existingDoc.data().athenaUserId;
  }

  // --- 2️⃣ Transaction-safe creation ---
  return await firestore.runTransaction(async tx => {
    ...
    // Core profile data (matches both Realtime DB and Firestore ideas)
    tx.set(userRoot.collection("profile").doc("core"), {
      displayName: discordUser.username,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      role: null,
      quizCompleted: false,
      linkedDiscordIds: [discordUser.id] // optional array like old Realtime DB
    });
...
/**
 * Optional: Add another Discord ID to an existing Athena user
 */
