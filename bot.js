...
const ai = genkit({
  ...
  // Upgrading to Gemini 2.0 Pro as requested
  // Note: Using the string identifier to ensure compatibility with the latest model releases
...
// ==========================
const requiredEnv = ["DISCORD_TOKEN", "GOOGLE_GENAI_API_KEY", "GCLOUD_PROJECT"];
...
let db;
try {
  const PROJECT_ID = process.env.GCLOUD_PROJECT || "athenaai-memory"; 
  let firebaseCreds;
...
/**
 * Syncs the detailed Users collection with underscore naming
 */
...
/**
 * Logs messages into platform-specific logs using underscore naming
 */
...
async function getAthenaResponse(userMessage, userId, username, timestamp, platform) {
  ...
  try {
    ...
    return "I'm having trouble thinking right now. (Genkit 2.0 Pro Error)";
  }
}
