...
// Using the official Google AI SDK directly for maximum stability
...
/**
 * Syncs the detailed Users collection
 */
...
/**
 * Logs messages into platform-specific logs
 */
...
async function getAthenaResponse(userMessage, userId, username, timestamp, platform) {
  ...
  try {
    // Format history for the official Google AI SDK
    const chat = model.startChat({
      history: [
        { role: "user", parts: [{ text: ATHENA_SYSTEM_PROMPT + " Acknowledge this persona." }] },
        { role: "model", parts: [{ text: "I am Athena. I understand my role as your wise advisor." }] },
        ...history.map(msg => ({
