...
// ESM module: manages an in-memory cache of "knowledge entries" backed by Firestore.
// Exports: initKnowledgeUpdater(firestore, options) -> { getEntry, searchEntries, getAll }
// The module polls Firestore every intervalMs and keeps the cache fresh.
// It is resilient: logs failures, backoffs, and keeps working.
...
// simple substring search over title/body; can be improved
...
// subscribe(listener) -> listener(entriesArray)
...
function _emit(entries) {
  for (const l of _listeners) {
    try { l(entries); } catch (err) { console.error("[Knowledge] listener error", err); }
  }
}
  ...
  (async function backgroundLoop() {
    while (!_stopped) {
      try {
        ...
        await wait(Math.min(60000, Math.max(5000, intervalMs / 6)));
      }
    }
    _running = false;
  })();
