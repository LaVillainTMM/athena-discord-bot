...
export async function syncLatestDojPressReleases(limit = 25) {
  ...
  for (const item of items) {
    try {
      ...
      console.error("[DOJ] Failed to store item:", item.title, err.message);
    }
  }
...
export async function searchAndStoreDoj(query, limit = 10) {
  ...
  for (const result of results.slice(0, 5)) {
    try {
      ...
      console.error("[DOJ] Failed to store search result:", err.message);
    }
  }
...
export async function storeDojResearchEntry({ title, content, source, redactionContext, redactionReason }) {
  try {
    ...
    console.log("[DOJ] Stored redaction research:", title);
    return true;
  } catch (err) {
    console.error("[DOJ] Failed to store research:", err.message);
    return false;
  }
}
