...
async function buildEntries(region, profile, weather) {
  ...
  const cActive   = Array.isArray(c?.active)   ? c.active
                   : (typeof c?.active   === "string" && c.active.trim()   ? [c.active.trim()]   : []);
  const cCanceled = Array.isArray(c?.canceled) ? c.canceled
                   : (typeof c?.canceled === "string" && c.canceled.trim() ? [c.canceled.trim()] : []);
  if (c && (cActive.length || cCanceled.length)) {
    const parts = [];
    if (cActive.length)   parts.push(`ACTIVE / SUCCESSFUL PROJECTS:\n- ${cActive.join("\n- ")}`);
    if (cCanceled.length) parts.push(`CANCELED / PAUSED PROJECTS:\n- ${cCanceled.join("\n- ")}`);
    tag(
      "construction",
