...
export async function enqueueSubject(subject, parentTitle = null) {
  ...
  try {
    ...
    console.log(`[DeepResearch] Queued: "${trimmed}"${parentTitle ? ` (from "${parentTitle}")` : ""}`);
    return true;
  } catch (err) {
...
/* ── Run the WHO/WHAT/WHERE/HOW dossier on a subject ── */
export async function researchSubject(subject) {
  const model = getResearchModel();
  const prompt =
    `Conduct a deep research dossier on: "${subject}"\n\n` +
...
export async function processSubject(subject, parentTitle = null) {
  console.log(`[DeepResearch] Researching: "${subject}"`);
  const facts = await researchSubject(subject);
  if (!facts.length) {
...
export function startDeepResearchWorker() {
  ...
  setInterval(async () => {
    try {
      ...
      const stored = await processSubject(data.subject, data.parentTitle);
